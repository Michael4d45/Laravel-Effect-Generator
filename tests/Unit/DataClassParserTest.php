...
function typeNodeToString(null|\PHPStan\PhpDocParser\Ast\Type\TypeNode $typeNode): null|string
{
    if ($typeNode === null) {
        return null;
    }
...
function typeNodeToStringRecursive(\PHPStan\PhpDocParser\Ast\Type\TypeNode $typeNode): string
{
    ...
    if ($typeNode instanceof \PHPStan\PhpDocParser\Ast\Type\GenericTypeNode) {
        ...
        return (
            typeNodeToStringRecursive($typeNode->type)
            . '<'
            . implode(', ', $args)
            . '>'
        );
    ...
    if ($typeNode instanceof \PHPStan\PhpDocParser\Ast\Type\ArrayShapeNode) {
        ...
        foreach ($typeNode->items as $item) {
            ...
            $items[] =
                ($key ? $key . ': ' : '')
                . $optional
                . typeNodeToStringRecursive($item->valueType);
        }
...
function getAttributeStrings(
    \EffectSchemaGenerator\Tokens\PublicPropertyToken $token,
    string $className,
): array {
    ...
    foreach ($attributes as $attribute) {
        ...
        if (empty($args)) {
            ...
            $strings[] =
                $attributeClassName . '(' . implode(', ', $formattedArgs) . ')';
        }
    }
...
it('captures array type from @param annotation in class docblock', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\UpdatePlaylistRequestWithParam::class);

    expect($classDefinition->publicProperties)->toHaveKey('question_ids');
});

it('parses AddressData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\AddressData::class);

    expect($classDefinition->publicProperties)->toHaveKeys([
        'street',
        'city',
        'state',
        'zipCode',
...
it('parses ApiResponseData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\ApiResponseData::class);
...
it('parses CategoryData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\CategoryData::class);
...
it('parses CollectionData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\CollectionData::class);
...
it('parses ComplexData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\ComplexData::class);
...
it('parses EdgeCaseData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\EdgeCaseData::class);
...
it('parses GameSessionData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\GameSessionData::class);
...
it('parses ProductData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\ProductData::class);
...
it('parses ProductImageData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\ProductImageData::class);
...
it('parses ProductReviewData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\ProductReviewData::class);
...
it('parses ProfileData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\ProfileData::class);
...
it('parses QuizModeData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\QuizModeData::class);

    expect($classDefinition->publicProperties)->toHaveKeys(['id', 'name']);
});

it('parses QuizRequestWithParam', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\QuizRequestWithParam::class);

    expect($classDefinition->publicProperties)->toHaveKey('quiz_modes');
});

it('parses QuizRequestWithVar', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\QuizRequestWithVar::class);

    expect($classDefinition->publicProperties)->toHaveKey('quiz_modes');
});

it('parses ScoringRuleData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\ScoringRuleData::class);
...
it('parses TaskData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\TaskData::class);
...
it('parses TreeNodeData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\TreeNodeData::class);
...
it('parses UpdatePlaylistRequestWithVar', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\UpdatePlaylistRequestWithVar::class);

    expect($classDefinition->publicProperties)->toHaveKey('question_ids');
});

it('parses UserData', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\UserData::class);
...
it('parses ClassWithOptionalTrait', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\ClassWithOptionalTrait::class);

    expect($classDefinition->publicProperties)->toHaveKeys([
        'traitOptionalProperty',
        'traitRequiredProperty',
        'classProperty',
    ]);
});
...
it('extracts attributes with arguments', function () {
    ...
    expect($attributeStrings)->toContain(
        'Spatie\LaravelData\Attributes\MapInputName("first_name")',
    );
    expect($attributeStrings)->toContain(
        'Spatie\LaravelData\Attributes\MapOutputName("givenName")',
    );

    $settingsProperty = $classDefinition->publicProperties['settings'];
    $settingsAttributeStrings = getAttributeStrings(
        $settingsProperty,
        $className,
    );
    expect($settingsAttributeStrings)->toContain(
        'Spatie\LaravelData\Attributes\WithCast("json")',
    );
});
...
it('extracts attributes without arguments', function () {
    ...
    $optionalProfileProperty =
        $classDefinition->publicProperties['optionalProfile'];
    $attributeStrings = getAttributeStrings(
        $optionalProfileProperty,
        $className,
    );
...
it('extracts phpdoc types from constructor doc comment', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\ConstructorParamData::class);
...
it('parses complex array shape with union types from @param annotation', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\ComplexMetadataData::class);
    ...
    expect($metadataPhpDocType)->toBe(
        'array<string, array{name: string, value: int|string|null}>',
    );
});
...
it('handles properties with null types', function () {
    ...
    $reflectionClass =
        new \ReflectionClass(\EffectSchemaGenerator\Tests\Fixtures\AddressData::class);
...
it('captures class attributes', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\OptionalAttributeData::class);

    expect($classDefinition->attributes)->toBeArray();
    expect($classDefinition->attributes)->toHaveCount(1);
    expect($classDefinition->attributes[0]->getName())
        ->toBe('Spatie\LaravelData\Attributes\Optional');
});

it('captures property attributes', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\OptionalAttributeData::class);

    $optionalProperty = $classDefinition->publicProperties['optionalProperty'];
    expect($optionalProperty->attributes)->toBeArray();
    expect($optionalProperty->attributes)->toHaveCount(1);
    expect($optionalProperty->attributes[0]->getName())
        ->toBe('Spatie\LaravelData\Attributes\Optional');

    $requiredProperty = $classDefinition->publicProperties['requiredProperty'];
    expect($requiredProperty->attributes)->toBeArray();
    expect($requiredProperty->attributes)->toBeEmpty();

    $nullableOptional = $classDefinition->publicProperties['nullableOptional'];
    expect($nullableOptional->attributes)->toHaveCount(1);
    expect($nullableOptional->attributes[0]->getName())
        ->toBe('Spatie\LaravelData\Attributes\Optional');
});

it('captures trait property attributes', function () {
    $classDefinition = $this->parser->parse(\EffectSchemaGenerator\Tests\Fixtures\ClassWithOptionalTrait::class);

    $traitOptional =
        $classDefinition->publicProperties['traitOptionalProperty'];
    expect($traitOptional->attributes)->toHaveCount(1);
    expect($traitOptional->attributes[0]->getName())
        ->toBe('Spatie\LaravelData\Attributes\Optional');

    $traitRequired =
        $classDefinition->publicProperties['traitRequiredProperty'];
    expect($traitRequired->attributes)->toBeEmpty();

    $classProperty = $classDefinition->publicProperties['classProperty'];
    expect($classProperty->attributes)->toBeEmpty();
});
...
it('formats attribute arguments with different value types', function () {
    ...
    expect(getAttributeStrings($stringProperty, $className))
        ->toContain(
            'EffectSchemaGenerator\Tests\Fixtures\TestAttribute("string_value")',
        );

    // Test integer argument
    $intProperty = $classDefinition->publicProperties['intArg'];
    expect(getAttributeStrings($intProperty, $className))
        ->toContain('EffectSchemaGenerator\Tests\Fixtures\TestAttribute(42)');

    // Test float argument
    $floatProperty = $classDefinition->publicProperties['floatArg'];
    expect(getAttributeStrings($floatProperty, $className))
        ->toContain('EffectSchemaGenerator\Tests\Fixtures\TestAttribute(3.14)');

    // Test boolean true argument
    $trueProperty = $classDefinition->publicProperties['trueArg'];
    expect(getAttributeStrings($trueProperty, $className))
        ->toContain('EffectSchemaGenerator\Tests\Fixtures\TestAttribute(true)');

    // Test boolean false argument
    $falseProperty = $classDefinition->publicProperties['falseArg'];
    expect(getAttributeStrings($falseProperty, $className))
        ->toContain(
            'EffectSchemaGenerator\Tests\Fixtures\TestAttribute(false)',
        );

    // Test null argument
    $nullProperty = $classDefinition->publicProperties['nullArg'];
    expect(getAttributeStrings($nullProperty, $className))
        ->toContain('EffectSchemaGenerator\Tests\Fixtures\TestAttribute(null)');

    // Test array argument
    $arrayProperty = $classDefinition->publicProperties['arrayArg'];
    expect(getAttributeStrings($arrayProperty, $className))
        ->toContain(
            'EffectSchemaGenerator\Tests\Fixtures\TestAttribute(["a", "b", "c"])',
        );

    // Test associative array argument
    $assocArrayProperty = $classDefinition->publicProperties['assocArrayArg'];
    expect(getAttributeStrings($assocArrayProperty, $className))
        ->toContain(
            'EffectSchemaGenerator\Tests\Fixtures\TestAttribute([key1: "value1", key2: 123])',
        );
    ...
    expect(getAttributeStrings($classProperty, $className))
        ->toContain(
            'EffectSchemaGenerator\Tests\Fixtures\TestAttribute("EffectSchemaGenerator\\\\Tests\\\\Fixtures\\\\UserData")',
        );
});
